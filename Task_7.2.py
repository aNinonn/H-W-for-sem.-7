...
# по номеру строки и столбца, т.е. функцию двух аргументов. 
...
def print_operation_table(operation, num_rows=6, num_columns=6):
    for x in range (1, num_rows):
        answer = " ".join([f"\t{operation(x,y)}" for y in range(1, num_columns+1)])
        print(answer)
        last_row = " ".join([f"\t{operation(num_rows,y) }" for y in range(1, num_columns+1)])
    return last_row
